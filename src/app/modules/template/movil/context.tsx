...
export const ContentProvider: FC<WithChildren> = ({children}) => {
    ...
    const uploadResource = async (file: any, option: number) => {
        setShowLoad(true)
        const url = `${process.env.REACT_APP_URLAWS}resource-${
            changeTypeEdit === 1 ? 'mobile' : 'web'
        }-${id}-${file.name}`
        const fileResource = {
            id_punto: tipo === 'punto' ? id : -1,
            id_sitio: tipo === 'sitio' ? id : idSitio,
    ...
    const uploadElement = async (data: any) => {
        ...
        setAllResourceElement(
            appendData(allResourcesElement, {...item, id_recurso: response.data.id_recurso})
        )
        handleClose(false)
        setShowLoad(false)
    }
    ...
    const getAllResources = async (type: number) => {
        ...
        const response: any = await postData('site/mobile/resource', {
            ...jsonData,
            tipo_elemento: 'multimedia',
        })
        const response2: any = await postData('site/mobile/resource', {
            ...jsonData,
            tipo_elemento: 'element',
        })
        if (response.data) {
    ...
    const destroyOneResource = async (id: number, tipo: number) => {
        try {
            ...
            const items = (tipo === 1 ? allResources : allResourcesElement).filter(
                (item: any) => String(item.id_recurso) !== String(id)
            )
            response &&
                swal({
